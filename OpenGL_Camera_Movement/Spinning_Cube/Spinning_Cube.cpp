...
int main()
{
	...
	vec3f camera_position = { 0.35f, 1.0f, -2.5f }; 
	Camera camera(90.0f, 0.2f, 8.0f, window.GetAspectRatio(), renderer.context); 
	camera.SetPosition(camera_position);

	// Register the camera's key handler as the callback
	window.SetKeyCallback([&camera](int key, int scancode, int action, int mods) 
	{
		camera.Callback_Key(key, scancode, action, mods);
	});
	...
	floor_plane.transform.Translate(0.0f, 0.0f, 0.0f);
	...
	cube_mesh.transform.Translate(0.75f, 0.75f, 0.75f);
	...
	triangle_3d_mesh.transform.Translate(-0.75f, 0.75f, -0.75f);
	...
	pyramid_mesh.transform.Translate(0.75f, 0.75f, -0.75f);
	...
	while (!window.ShouldWindowClose()) 
	{
		...
		if (pyramid_timer.IsTimerExpired())
		{
			...
			pyramid_mesh.transform.Translate(0.75f, (0.75f + (sin(TO_RADIANS(translation_y_Pyramid)) / 2.0f)), -0.75f);
